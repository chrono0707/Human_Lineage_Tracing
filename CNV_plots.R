...
#event number per CNVtype (EDFig 5a)
...
m_event_tbl %<>% 
  mutate(type2=replace(type2, type2=="WCA", "Whole-chromosome gain")) %>%
  mutate(type2=replace(type2, type2=="WCD", "Whole-chromosome loss")) %>% 
  mutate(type2=replace(type2, type2=="AA",  "Arm-level gain")) %>%
  mutate(type2=replace(type2, type2=="AD",  "Arm-level loss")) %>% 
  mutate(type2=replace(type2, type2=="ICA", "Intrachromosomal gain")) %>%
  mutate(type2=replace(type2, type2=="ICD", "Intrachromosomal loss"))
  
m_event_tbl$type2 <- factor(m_event_tbl$type2, levels = c('Whole-chromosome gain',
                                                          'Whole-chromosome loss',
                                                          'Arm-level gain',
                                                          'Arm-level loss',
                                                          'Intrachromosomal gain',
                                                          'Intrachromosomal loss'))
...
ggplot(m_event_tbl, aes(x=type2, y=count))+
  ...
  theme(axis.text.x = element_text(angle=45, hjust=1),
        plot.margin = unit(c(0,0,0,2), "cm"), text=element_text(size=15),
...
#event number per chromosome (EDFig 5b)
colnames(fm_lng_dt)
tmp_tbl <- fm_lng_dt %>% select(deadbody, lineage_id2, starts_with('whole_chrom_'), starts_with('q_arm_'), starts_with('intra_chrom_')) 
tmp_tbl %<>% mutate_at(3:ncol(.), function(x) replace(x, x=="N", 0))
tmp_tbl %<>% mutate_at(3:ncol(.), function(x) replace(x, x=="Y", 1))
...
ggplot(fm_tmp_tbl, aes(x=chrom, y=count, fill=deadbody))+
  geom_bar(stat = 'identity')+
  scale_fill_manual(values = db_pal)+
  xlab("Chromosome")+ylab("Number of events")+
