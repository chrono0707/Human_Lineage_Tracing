...
for (this_db in c('DB3','DB6','DB8','DB9','DB10')){
  ...
  tmp_tbl <- left_join(tmp_tbl,pmeta_dt %>% dplyr::select(sample_id, dominant_layer2, anatomy_class1)) %>% separate(lineage_id2, c('deadbody','lineage_id'), sep="_") %>% 
    spread(key=lineage_id, value=meanVAF) %>% mutate(L1ratio=L1/(L1+L2), L2ratio=L2/(L1+L2)) %>% filter(is.na(L1ratio)==F & is.na(L2ratio) == F)
  ...
  if(this_db=="DB8") x_order <- tmp_tbl %>% arrange(L2ratio) %>% pull(sample_id)
  else               x_order <- tmp_tbl %>% arrange(L1ratio) %>% pull(sample_id)

  tmp_tbl %<>% mutate(L1ratio=L1ratio*100, L2ratio=L2ratio*100, L1=L1*100, L2=L2*100) 
  ...
  tmp_tbl %<>% ungroup() %>% arrange(L1ratio) %>% mutate(sample_id=factor(sample_id, levels=x_order))
  
  if(this_db == "DB3")  { exp_L1 <- 66.7/2; exp_L2 <- 33.3/2; exp_L1ratio <- 66.7 }
  if(this_db == "DB6")  { exp_L1 <- 86.6/2; exp_L2 <- 13.4/2; exp_L1ratio <- 86.6 }
  if(this_db == "DB8")  { exp_L1 <- 59.1/2; exp_L2 <- 40.9/2; exp_L1ratio <- 59.1 }
  if(this_db == "DB9")  { exp_L1 <- 72.4/2; exp_L2 <- 27.6/2; exp_L1ratio <- 72.4 }
  if(this_db == "DB10") { exp_L1 <- 68.4/2; exp_L2 <- 31.6/2; exp_L1ratio <- 68.4 }
  
  if(this_db=="DB8") { 
    g1 <- tmp_tbl %>% 
      ggplot(aes(x=sample_id, y=L2)) + 
      geom_bar(stat="identity", width=1.0, fill=rgb(249/255, 205/255, 45/255)) + 
      geom_hline(yintercept=exp_L1, color="red", linetype="dashed")+
      coord_flip() + 
      ...
      theme(axis.title=element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank(), panel.grid = element_blank(),
            plot.margin = unit(c(0,0.2,0,0), "cm"), axis.line.x=element_line(color="black"), text=element_text(size=25))
    
    g2 <- tmp_tbl %>% 
      ggplot(aes(x=sample_id, y=L1)) + 
      geom_bar(stat="identity", width=1.0, fill=rgb(94/255, 133/255, 218/255)) + 
      geom_hline(yintercept=exp_L2, color="red", linetype="dashed")+
      coord_flip() + 
      scale_y_reverse(limits=c(100,0), expand=c(0,0), breaks=c(50,100)) + 
      #scale_y_continuous(limits=c(0,100), expand=c(0,0), breaks=c(0, 50,100)) + 
      theme(axis.title=element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank(), panel.grid = element_blank(),
            plot.margin = unit(c(0,0,0,0.4), "cm"), axis.line.x=element_line(color="black"), text=element_text(size=25))
  ...
  #p_list[[n]] <- plot_grid(g2, g1, g3, g4, g5, nrow=1, rel_widths = c(0.2, 0.2, 0.5, 0.05, 0.05), align = "h")
  p_list[[n]] <- plot_grid(g2, g1, g4, g5, nrow=1, rel_widths = c(0.45, 0.45, 0.05, 0.05), align = "h")
